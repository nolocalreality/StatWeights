...
local function updateTextfield(widget, id, config, parent)
  ...
  Library.LibExtendedWidgets.SetBorder(textfield, 1, 0.5, 0.5, 0.5, 1)
  if config.get then
...
local function createCheckbox(id, config, parent)
  local widget = UI.CreateFrame("ExtendedCheckBox", "Checkbox_" .. id, parent)
...
function Library.LibExtendedWidgets.Layout(configTable, parent)
  ...
  for _, id in ipairs(sortedIDs) do
    ...
    if config.tooltipText ~= nil then
      if tooltip == nil then
        tooltip = UI.CreateFrame("ExtendedTooltip", "LayoutTooltip", parent)
