LibExtendedWidgets is a library which provides a set of simple widgets not provided by the official Rift API. It integrates into the UI.CreateFrame function, adding several new frame types:

* ExtendedCheckBox (checkbox with a label)
...
* ExtendedTooltip (mouseover popup frame with text)
...
Frame Type: ExtendedCheckBox
==========================

ExtendedCheckBox displays a checkbox with a label. It supports the following functions from RiftCheckbox: GetChecked, SetChecked, GetEnabled, SetEnabled. It also supports the following events from RiftCheckbox: CheckboxChange.
...
It has the same functions as ExtendedCheckBox except GetChecked, SetChecked.
...
Frame Type: ExtendedTooltip
=========================

ExtendedTooltip is a frame that can be configured to popup when the user mouses over a frame.
...
**Library.LibExtendedWidgets.RadioButtonGroup(name)**  
...
**Library.LibExtendedWidgets.Layout(configTable, parent)**  
...
**Library.LibExtendedWidgets.SetBorder(type, frame, ...)**  
...
------
    ...
    Library.LibExtendedWidgets.Layout(configTable, SWT_Window:GetContent())
...
-------
    ...
    local tooltip = UI.CreateFrame("ExtendedTooltip", "MyTooltip", parent)
...
----------------
    ...
    local radioButtonGroup = Library.LibExtendedWidgets.RadioButtonGroup("MyRadioButtonGroup")
